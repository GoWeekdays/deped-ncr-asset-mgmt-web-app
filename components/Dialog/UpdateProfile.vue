<template>
  ...
  >
    <v-row no-gutters align="center" class="ma-0">
      <v-col class="pa-0">
        <p class="font-weight-bold">{{ alertMessage }}</p>
        <p v-if="showSubMessage" class="text-subtitle-2">{{ subMessage }}</p>
      </v-col>
    </v-row>
...
const props = defineProps({
  ...
  updateType: {
    ...
    default: "profile",
    validator: (value) =>
      ["profile", "email", "password", "photo"].includes(value),
  },
});
...
const showSubMessage = computed(() => props.updateType === "email");
const subMessage = computed(() =>
  props.updateType === "email" ? "Please check your email for verification" : ""
);
